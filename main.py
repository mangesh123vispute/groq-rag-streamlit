...
class AgenticRAG:
    ...
    def __init__(self, urls=None):
        ...
        # urls is None → use defaults; urls is [] → local files only (empty urls.txt)
        if urls is None:
            self.urls = list(Config.DEFAULT_URLS)
        else:
            self.urls = urls
    ...
    def _setup_vectorstore(self):
        ...
        print(f"📄 Loading {len(self.urls)} URL(s) + files under {Config.DATA_DIR} ...")
        documents = self.doc_processor.process_sources(self.urls, Config.DATA_DIR)
